...
# vk_api ставится из requirements.txt (пакет vk-api). Ловим ImportError здесь,
# а не на старте бота: иначе отсутствие пакета на уже установленном сервере
# роняло бы весь бот, хотя сервис просто выключен.
try:
    import vk_api
...
class VkService(_Service):
    def __init__(self, bot: Bot, config: VkModel) -> None:
        ...
        # Без пакета или без токена сервис не поднимается: VK отдаёт музыку
        # только по токену с доступом к аудио.
        self.is_enabled = bool(config.enabled and config.token and vk_api)
        if config.enabled and not config.token:
            self.error_message = "Не указан токен VK (services.vk.token)"
        elif config.enabled and not vk_api:
            self.error_message = "Пакет vk-api не установлен (pip install vk-api)"
        else:
            self.error_message = ""
    ...
    def initialize(self) -> None:
        if not vk_api:
            raise errors.ServiceError("Пакет vk-api не установлен (pip install vk-api)")
        http = requests.Session()
        http.headers.update(
            {
                "User-agent": "VKAndroidApp/6.2-5091 (Android 9; SDK 28; samsung exynos 7870; samsung j6lte; 720x1450)"
            }
        )
    ...
    def search(self, query: str, limit: Optional[int] = None) -> List[Track]:
        count = limit if limit else 300
        results = self.api.audio.search(q=query, count=count, sort=0)
